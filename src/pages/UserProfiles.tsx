...
const CLOUDINARY_UPLOAD_URL =
  "https://api.cloudinary.com/v1_1/dupjdmjha/image/upload"; // Replace YOUR_CLOUD_NAME
...
export default function EnzaladaCompanyProfile() {
  ...
  const uploadImageToCloudinary = async (
    ...
    if (CLOUDINARY_UPLOAD_URL.includes("YOUR_CLOUD_NAME")) {
      setError(
        "Cloudinary not configured. Please replace placeholder settings."
      );
      return null;
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (err.message === "Logo upload failed, stopping profile save.") {
        ...
        setError(
          "Failed to save profile. Check security rules for WRITE permissions on 'company_settings'."
        );
      }
    } finally {
      setSaving(false);
  ...
  return (
    <>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] lg:p-6">
        ...
        {/* Status Messages */}
        {error && (
          <div className="text-center py-2 px-4 mb-4 text-error-500 border border-error-500 bg-red-100 rounded-lg">
            {error}
          </div>
        )}
        {successMessage && (
          <div className="text-center py-2 px-4 mb-4 text-success-500 border border-success-500 bg-green-100 rounded-lg">
            {successMessage}
          </div>
        )}
