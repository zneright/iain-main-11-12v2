...
// FIREBASE IMPORTS
// -------------------------------------------------------------------------
// ⭐ ADDED: doc and updateDoc for updating the read status
...
export default function NotificationsPage() {
  ...
  // =================================================================
  // ⭐ NEW ACTION: Mark Single Notification as Read
  // =================================================================
  const handleMarkAsRead = async (notifId: string) => {
    ...
    try {
      // 1. Update Firestore: Target the document using the notification ID
      const notifDocRef = doc(db, "notifications", notifId);
      await updateDoc(notifDocRef, {
        read: true,
      });

      // 2. Update local state: Find the notification and set its read status to true
          ...
          {notifications.length === 0 ? (
            ...
            notifications.map((notif) => (
              ...
              >
                ...
                {!notif.read && (
                  <button
                    onClick={() => handleMarkAsRead(notif.id)} // ⭐ Uses new async handler
