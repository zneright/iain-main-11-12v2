...
interface Notification {
  ...
  name: string; // Source user/system (Will now map to title if data.sourceUser is missing)
  title: string; // Main notification title (mapped from Firestore title)
  description: string; // Full message body (mapped from Firestore description)
  category: string;
  time: string; // Formatted time ago
  read: boolean;
  scheduledDate: string; // Raw date string (YYYY-MM-DD)
}

// ⭐ IMPROVED HELPER 1: Formats a Date object to "X [unit] ago" or a full date.
const formatTime = (timestamp: Date) => {
  ...
  // If older than a day, use a standard date format
...
// ⭐ NEW HELPER 2: Formats the scheduled date string (YYYY-MM-DD)
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      const fetchedNotifs: Notification[] = querySnapshot.docs.map(doc => {
        ...
        return {
          id: doc.id,
          // ⭐ ACTION: Removed "System Scheduler" fallback. Now defaults to the notification title.
          name: data.sourceUser || data.title || "Notification Source",
            ...
            notifications.map((notif) => (
              ...
              >
                <div className="flex-1">
                  {/* 1. Main Title/Source and Scheduled Date */}
                  <p className="text-gray-700 dark:text-gray-300">
                    <span className="font-semibold text-gray-900 dark:text-white">
                      {/* Using name: which is now the source user or the title */}
                      {notif.name}
                    </span>{" "}
                  ...
                  {/* 2. Description (Full Message Body) */}
