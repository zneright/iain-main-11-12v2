...
// FIREBASE IMPORTS
// -------------------------------------------------------------------------
// Added where and getDoc for targeted lookup
...
interface Notification {
  id: string;
  name: string; // Source user/system (maps to fetched applicant name/title)
  title: string;
  description: string;
  category: string;
  ...
  // ⭐ ADDED: Recipient details for display
...
export default function NotificationsPage() {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      // ⭐ CRITICAL STEP 1: Perform lookups for each targetUid
      const notificationsWithApplicantData = await Promise.all(
        rawNotifs.map(async ({ id, data }) => {
          ...
          return {
            ...
            // ⭐ CRITICAL STEP 2: Map fetched recipient data
