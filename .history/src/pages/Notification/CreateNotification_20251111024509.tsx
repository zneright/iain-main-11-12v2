...
// Interface for the notification form data 
interface NotificationData {
  title: string;
  description: string;
  date: string;
  // ⭐ ADDED: Field for Notification Type
  type: string;
  targetUid: string;
}
...
export default function CreateNotif() {
  ...
  // Handler for selecting a SINGLE applicant
  ...
  useEffect(() => {
    const fetchPendingApplicants = async () => {
      try {
        // Queries 'accounts' where status == 'Pending'
  ...
  const handleCreateNotif = async () => {
    ...
    // ⭐ UPDATED VALIDATION: Check for type field
    ...
    try {
      ...
      await addDoc(notifCollectionRef, {
        ...
        // ⭐ ADDED: Save the notification type
        type: formData.type,
        status: 'scheduled',
  ...
  return (
    <ComponentCard title="Notification Details">
      ...
      {/* ⭐ NEW FIELD: Notification Type Select */}
      <div className="mb-4">
      ...
      <div className="flex justify-end">
        <Button
          ...
          // Disable if loading OR if required fields/target UID is missing
